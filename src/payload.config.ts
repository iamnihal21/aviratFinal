...
export default buildConfig({
  ...
  // Use the environment variable for production, fallback for local dev
  serverURL: process.env.PAYLOAD_PUBLIC_SERVER_URL || 'http://localhost:3000',
  ...
  // Ensure this is never empty in production
  secret: process.env.PAYLOAD_SECRET || 'YOUR_FALLBACK_SECRET_FOR_LOCAL_ONLY',
  ...
  plugins: [
    s3Storage({
      collections: {
        media: {
          // This generates the absolute URL pointing directly to Supabase
          generateFileURL: ({ filename }) => {
            return `${process.env.SUPABASE_S3_ENDPOINT}/${process.env.SUPABASE_BUCKET}/${filename}`
          },
        },
      },
      bucket: process.env.SUPABASE_BUCKET!,
