...
import { Briefcase, TrendingUp, DollarSign, Building2, MapPin, Award, Sparkles, ChevronRight } from 'lucide-react'
...
const { ref: heroRef } = useScrollAnimation({ threshold: 0.1, triggerOnce: true })
  ...
  return (
    <main className="min-h-screen bg-white text-gray-950 ">
      ...
      <section className="py-32 bg-gray-950 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
          <div className="text-center mb-20">
             <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter mb-4">Placement <span className="text-secondary italic">Metrics.</span></h2>
                  ...
                  <Card className="bg-white/5 border border-white/10 rounded-[3rem] overflow-hidden group hover:bg-white/10 hover:border-primary/50 transition-all duration-500 h-full">
                    ...
                    <CardContent className="p-10">
                      <div className="flex justify-between items-start mb-8">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 italic">#{index + 1}</span>
                      </div>
                      <div className="text-5xl font-black text-white tracking-tighter mb-4">{item.count}</div>
                      <h3 className="text-xl font-black text-white mb-4 group-hover:text-secondary transition-colors">{item.title}</h3>
      ...
      <section ref={recruitersRef} className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-4">Industry Partners</h2>
              <p className="text-gray-500 font-medium">Top-tier organizations where our alumni are making an impact today.</p>
            </div>
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-primary">
              Scroll to see more <ChevronRight className="w-4 h-4" />
            </div>
          </div>
          ...
          >
            {recruiters.map((recruiter, i) => (
              <motion.div key={i} variants={fadeInUp}>
                <Card className="bg-gray-50 border-transparent rounded-[2.5rem] hover:bg-white hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 group border-2 hover:border-primary/10 h-full">
                  <CardContent className="p-8 text-center flex flex-col items-center justify-center min-h-[220px]">
                    <div className="h-20 w-20 mx-auto mb-6 bg-white rounded-[1.5rem] flex items-center justify-center group-hover:shadow-lg transition-all duration-500 overflow-hidden border border-gray-100">
                      {recruiter.logo ? (
                        <Image
                    ...
                    <h3 className="text-sm font-black tracking-tight mb-2 group-hover:text-primary transition-colors">{recruiter.name}</h3>
