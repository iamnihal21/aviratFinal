...
import {
  Calendar, CheckCircle2, FileText, GraduationCap,
  Users, Trophy, Clock, Sparkles, ArrowRight, ShieldCheck, MapPin
} from 'lucide-react'
...
export default function AdmissionsClient({ data }: { data: any }) {
  ...
  const getIcon = (type: string) => {
    const icons: any = { 
      graduation: GraduationCap, users: Users, trophy: Trophy, 
      clock: Clock, file: FileText, check: CheckCircle2, shield: ShieldCheck 
    }
  ...
  return (
    <main className="min-h-screen bg-white text-gray-950">
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 text-center flex flex-col items-center">
          <motion.div ref={heroRef} initial="hidden" animate="visible" variants={staggerChildren} className="max-w-4xl">
            <motion.div variants={fadeInUp} className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-full mb-8 border border-gray-200 shadow-sm">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em]">Admissions Open 2026</span>
            </motion.div>
            ...
            <motion.p variants={fadeInUp} className="text-xl text-gray-500 leading-relaxed mb-12 max-w-2xl mx-auto font-medium">
              Join a legacy of excellence in Ahmedabad. We value curiosity, leadership, and the drive to innovate in the digital age.
            </motion.p>
          </motion.div>
        </div>
      </section>
      ...
      {/* <section className="relative -mt-20 z-10 mb-24">
            ...
            {data?.highlights?.map((item: any, i: number) => {
              ...
              return (
                <motion.div key={i} variants={fadeInUp}>
                  <Card className="bg-bg-secondary/20 backdrop-blur-xl border-none shadow-2xl hover:shadow-primary/10 transition-all duration-500 rounded-[2rem] group">
                    <CardContent className="p-8 text-center">
                      <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-primary group-hover:text-white transition-colors duration-500">
              ...
              {activeTab === "process" && (
                ...
                >
                  <div className="text-center mb-16">
                    ...
                    <p className="text-gray-500 font-medium">Follow these five simple steps to secure your seat at Avirat.</p>
                  </div>
                  ...
                  {['undergraduate', 'graduate'].map((category) => (
                    <div key={category} className="space-y-8">
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <h2 className="text-3xl font-black tracking-tight capitalize">{category} Intake</h2>
                      </div>
                      
                      <div className="space-y-6">
                        {data?.deadlines?.[category]?.map((item: any, i: number) => (
                          <Card key={i} className="group overflow-hidden rounded-[2rem] border-2 border-gray-50 hover:border-primary/20 transition-all duration-500 bg-secondary/20">
                            <CardContent className="p-10">
                              <div className="flex justify-between items-center mb-8">
                                <h3 className="text-xl font-black tracking-tight">{item.type}</h3>
                                <span className="px-4 py-1 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase tracking-widest">Active</span>
                              </div>
                              <div className="grid grid-cols-2 gap-8">
                                <div className="space-y-2">
                                  ...
                                  <div className="text-lg font-bold text-gray-900">{item.deadline}</div>
                                </div>
                                <div className="space-y-2">
                                  <div className="flex items-center gap-2 text-gray-400 text-[10px] font-black uppercase tracking-widest">
                                    <CheckCircle2 className="h-3 w-3" /> Result
                                  </div>
                                  <div className="text-lg font-bold text-gray-900">{item.notification}</div>
                                </div>
                              </div>
                            </CardContent>
                          </Card>
